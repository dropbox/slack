...
func (api *Client) StartRTMContext(ctx context.Context) (info *Info, websocketURL string, err error) {
	...
	err = api.callSlackMethod(ctx, "rtm.start", url.Values{"token": {api.authConfig.AccessToken}}, response)
	if err != nil {
		return nil, "", err
	}
...
func (api *Client) ConnectRTMContext(ctx context.Context) (info *Info, websocketURL string, err error) {
	response := &infoResponseFull{}
	err = api.callSlackMethod(ctx, "rtm.connect", url.Values{"token": {api.authConfig.AccessToken}}, response)
	if err != nil {
